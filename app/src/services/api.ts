...
class ApiService {
  ...
  // ======================
  // FORGOT PASSWORD / PASSWORD RESET (OTP-based)
  // ======================
  ...
  /**
   * Step 2: Verify OTP + Reset Password in one call
  ...
  async deleteFile(fileId: number, permanent = false) {
    ...
    const response = await this.client.delete(
      `/files/${fileId}?permanent=${permanent}`
    )
   ...
   * POST /api/share
   * Body: { fileId, isPublic, permission }
   */
  ...
  async getMyActivityLogs(page = 0, size = 20) {
    ...
    const response = await this.client.get(
      `/activity-logs/my?page=${page}&size=${size}`
    )
